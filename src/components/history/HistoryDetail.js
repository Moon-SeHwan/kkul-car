...
const FareModal = forwardRef((props, ref) => {
  const { fare, onPlusClick, onMinusClick } = props

  return (
    <Modal>
      <div className="inBox">
        <input type="text" ref={ref} value={fare} readOnly />
        <button className="btn plus" onClick={() => onPlusClick()}>+</button>
        <button className="btn minus" onClick={() => onMinusClick()}>-</button>
      </div>
    </Modal>
  )
})
...
const HistDetail = () => {
  ...
  const addFare = () => {
    if (window.confirm("운송비를 수정하시겠습니까?")) {
      const additionalFare = parseInt(fareRef.current.value) - transitFare
      
      setCargoInfo({ ...cargoInfo, additionalFare: additionalFare })
  ...
  const handleMinusClick = () => {
    const total = transitFare + additionalFare
    
    if(parseInt(fareRef.current.value) - 5000 >= total) {
      fareRef.current.value = parseInt(fareRef.current.value) - 5000
    } else {
      alert('현재 총 운송비보다 적을 수 없습니다.')
    }
  }
  ...
  return (
    <section className="contentSection">
      ...
      <FareModal ref={fareRef} fare={transitFare + additionalFare} onPlusClick={() => handlePlusClick()} onMinusClick={() => handleMinusClick()} />
    </section>
  )
}
